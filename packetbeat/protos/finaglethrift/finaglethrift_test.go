...
func TestThrift_readMessageBegin(t *testing.T) {
	...
	//
	//
	//data, _ = hex.DecodeString("d4ee072bfae0b8e8564a52980800450000600faf40004006dc06c0a8c7d2c0a805bfdc4b5190659f1c87aa27407a80181015980900000101080a1a779a0313a3485400000028800100010000001b5f5f63616e5f5f66696e61676c655f5f74726163655f5f76335f5f0000000000")
	data = []byte{0, 0, 0, 40, 128, 1, 0, 1, 0, 0, 0, 27, 95, 95, 99, 97, 110, 95, 95, 102, 105, 110, 97, 103, 108, 101, 95, 95, 116, 114, 97, 99, 101, 95, 95, 118, 51, 95, 95, 0, 0, 0, 0, 0}
	stream = ThriftStream{data: data, message: new(ThriftMessage)}
	m = stream.message
	ok, complete = thrift.readMessageBegin(&stream)

	fmt.Println("ok: [" , ok, "]  complete: [", complete, "]  m.Method: [", m.Method, "]  parseOffset: [" , stream.parseOffset , "]  data.len: [", len(data), "]" )
...
func TestThrift_thriftMessageParser(t *testing.T) {
	...
	//data = []byte{0, 0, 0, 40, 128, 1, 0, 1, 0, 0, 0, 27, 95, 95, 99, 97, 110, 95, 95, 102, 105, 110, 97, 103, 108, 101, 95, 95, 116, 114, 97, 99, 101, 95, 95, 118, 51, 95, 95, 0, 0, 0, 0, 0}
	//data,_ = hex.DecodeString("")
	data,_ = hex.DecodeString("")
	stream = ThriftStream{data: data,tcptuple: new(common.TcpTuple), message: new(ThriftMessage)}
	m = stream.message
	m.FrameSize = 157- 133
	stream.parseOffset = 4 + 133
	ok, complete = thrift.messageParser(&stream,uint8(1))

	fmt.Println("ok: [" , ok, "]  complete: [", complete, "]  m.Method: [", m.Method, "]  parseOffset: [" , stream.parseOffset , "]  data.len: [", len(data), "],  frameSize: [", stream.message.FrameSize, "]" )

	//data = []byte{0, 0, 0, 40, 128, 1, 0, 1, 0, 0, 0, 27, 95, 95, 99, 97, 110, 95, 95, 102, 105, 110, 97, 103, 108, 101, 95, 95, 116, 114, 97, 99, 101, 95, 95, 118, 51, 95, 95, 0, 0, 0, 0, 0}
	//data,_ = hex.DecodeString("0000001900800100020000000b66696e61676c6550696e670000000000")

	//stream = ThriftStream{data: data,tcptuple: new(common.TcpTuple), message: new(ThriftMessage)}
	//m = stream.message
	//m.FrameSize = 25-1
	//stream.parseOffset = 4 + 1
	//ok, complete = thrift.messageParser(&stream,uint8(0))
	//
	//fmt.Println("ok: [" , ok, "]  complete: [", complete, "]  m.Method: [", m.Method, "]  parseOffset: [" , stream.parseOffset , "]  data.len: [", len(data), "],  frameSize: [", stream.message.FrameSize, "]" )
...
func TestThrift_mapStr(t *testing.T){
	event := common.MapStr{}
	var ip uint16 = 65
	event["client_ip"] = ip
	//tempBuf := bytes.Buffer{}
	//tempBuf.WriteString((event["client_ip"].(uint16)))
	//tempBuf.WriteString(UNITED_FIELD_SPLIT_FLAG)

	//event["united"] = string(e) + "|"
	//fmt.Println("===================", strconv.FormatUint(event["client_ip"].(uint16),10))
	...
	fmt.Println("===================", strings.Join(united, "|"))
